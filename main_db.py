...
class MainWindow:
    ...
    def __init__(self):
        # Store today's date
        self.today = date.today()+timedelta(days=6)
    ...
    def entryButton(self, enterPressed=True):
        ...
        ids_list = None
        # search for  reg_id
        ids_list = re.findall(rf"\s({reg_id_entry})\s", self.reg_id)
    ...
    def saveEntries(self):
        ...
        # Connect to server
        # Load server and database details from the JSON file
        ...
        try:
            ...
            cursor = conn.cursor()
            print(daily_data)
            # Iterate through the daily_data list and insert each row into the database
            for row in daily_data:
                sql_insert = f"""
                                       INSERT INTO {dailydata} (regid, name, [date], intime, outime)
